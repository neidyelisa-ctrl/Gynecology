# =============================================================================
# Script 21 — Voltando à abordagem DEG -> GO/KEGG (scripts 06/08/14), agora
# preenchendo a combinação que faltava: Chen 2006 (SUI) x GSE12852
# (SEGUNDO dataset de POP, ligamento uterossacral - ver script 16).
#
# Com isso, fecha-se a matriz completa 2 (POP: GSE53868, GSE12852) x 3
# (SUI: Chen2003, Chen2006, Wei2020) de cruzamentos DEG->GO/KEGG. O script
# 23 consolida os 6 resultados e procura temas que se repetem em mais de
# um par POP x SUI - o critério mais rigoroso de robustez deste projeto.
# =============================================================================
...
cat("Chen 2006 - genes candidatos:", nrow(chen), "\n\n")
...
write.csv(cross, "results/21_cruzamento_Chen2006_x_GSE12852_genes.csv", row.names = FALSE)

intersecao_estrita <- subset(cross, DEG_POP_FDR05_logFC1)
cat("=== Interseção ESTRITA: DEG(GSE12852) ∩ Chen2006 ===\n")
...
cat("=== Concordância de direção (Chen2006 x GSE12852) ===\n")
...
universe <- pop$Gene
cat("=== GO/KEGG nos", nrow(concordantes), "genes concordantes (Chen2006 x GSE12852) ===\n")
...
if (!is.null(go_b)) {
  ...
  write.csv(go_b, "results/21_GO_BP_concordantes.csv", row.names = FALSE)
...
if (!is.null(kegg_b)) {
  write.csv(kegg_b, "results/21_KEGG_concordantes.csv", row.names = FALSE)
