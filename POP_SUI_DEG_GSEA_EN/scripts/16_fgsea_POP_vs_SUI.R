...
## --- 7c: dedicated barplot of EVERY pathway significant in BOTH diseases --
## Built directly from kegg_both_significant (Part 5) - NOT a top-N-by-
## p-value cut of anything, so unlike 7a/7b above, a pathway here can never
## be crowded out by other, more extreme pathways elsewhere in the genome.
## The two barplots above rank ALL of POP's (or SUI's) significant pathways
## by p-value and show the top 15 per band - correct for "what stands out
## most in POP/SUI on its own", but a pathway that matters for the CROSS-
## DISEASE comparison specifically (i.e. it made the strict double-FDR<0.25
## bar) can still rank outside the top 15 within its own band if enough
## OTHER, unrelated pathways (e.g. broad housekeeping/neurodegenerative
## pathways sharing ribosomal genes) happen to have even smaller p-values.
## This chart exists specifically so that never happens: every pathway
## significant in both diseases is guaranteed a bar here, always.
...
## --- 7d: shared-pathway NES heatmap (pathways significant in BOTH) --------
if (nrow(kegg_both_significant) > 0) {
