...
**Full KEGG pathway names, not just numeric IDs.** Every figure and results table now shows the pathway's real name (e.g. "KEGG 04512 - ECM-receptor interaction"), not a bare ID. This is driven by `data/kegg_pathway_names.csv`, a single 223-row table covering every KEGG pathway ID that appears anywhere in this project's results, originally assembled from standard KEGG pathway-map nomenclature offline (this environment has no live internet access to KEGG.jp - confirmed by direct test, `curl` to `string-db.org`, `bioconductor.org`, and `targetscan.org` all return a proxy-level connection refusal, not a slow/failed request). All six analysis scripts (`POP_SUI_analysis.R`, `02`-`06`) load this one shared table instead of each keeping its own partial, inconsistent copy - one source of truth instead of six chances to disagree. `scripts/relabel_KEGG_pathways.R` re-applies the table to every already-saved results file and redraws the affected figures **without recomputing any statistic** (NES, p-values, FDR, DEG calls are untouched - only the human-readable name changes); run it any time the name table is edited, instead of re-running a 10-20 minute permutation loop.

**Independently verified against the live KEGG database.** The user downloaded the official current pathway list directly from `https://rest.kegg.jp/list/pathway/hsa` (from a machine with internet access, since this environment has none) and sent it back for a full cross-check against all 223 rows. Result: 205/220 matched exactly; the other 15 were mostly punctuation/wording-only differences (e.g. missing comma, "Alzheimer's disease" vs KEGG's own "Alzheimer disease") now corrected to match KEGG's exact current wording, plus **5 pathways KEGG has substantively renamed** since the version frozen in `org.Hs.eg.db`, also corrected:

| ID | Old (offline-assembled) name | Current official KEGG name |
|---|---|---|
| 03013 | RNA transport | Nucleocytoplasmic transport |
| 04142 | Lysosome | Lysosome biogenesis |
| 04145 | Phagosome | Phagocytosis |
| 04514 | Cell adhesion molecules | Cell adhesion molecule (CAM) interaction |
| 04666 | Fc gamma R-mediated phagocytosis | Fc gamma R-mediated phagosome formation |
...
**A bigger, related limitation this comparison surfaced**: the official KEGG pathway list currently has ~372 human pathways, but `org.Hs.eg.db` - the package that supplies pathway *gene membership* (not just names) for every GSEA run in this project - only knows **229 KEGG pathways in total** (confirmed directly: `length(AnnotationDbi::keys(org.Hs.eg.db, keytype="PATH"))` returns 229). This is not a bug in this project's code; KEGG restricted free, open redistribution of its full gene-to-pathway mapping around 2011, and Bioconductor's `org.Hs.eg.db` has used that last-open snapshot ever since (a fully current mapping requires a paid KEGG FTP subscription). This project's `kegg_pathway_names.csv` (223 rows) was never meant to cover all 372 - it only ever needed to cover the pathways this project could possibly test, which was capped at 229 from the start by `org.Hs.eg.db` itself, independent of the name-only audit above. **This will also affect script 11's Part 7 comparison**: `msigdbr`'s `KEGG_LEGACY` collection is, for the same licensing reason, the same older restricted snapshot MSigDB was able to redistribute - so a pathway count near ~200 (not 372) in that comparison is expected, not a sign something went wrong, and means the comparison is apples-to-apples (same restricted pathway universe on both sides) rather than unfairly compared against a more complete source.
